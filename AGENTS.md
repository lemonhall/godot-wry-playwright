...
- `scripts\run_tests.ps1 -SkipV3RuntimeCoverageCheck` (skip v3 runtime API-coverage gate when debugging test infra only)
- `scripts\run_tests.ps1 -EmitCoverageReport` (运行覆盖门禁并自动刷新 `docs/plan/v3-runtime-coverage-matrix.{md,json}`)
...
- `python3 scripts/check_v3_runtime_test_coverage.py` (强制门禁：M3.1/M3.2 已实现 API 必须有 runtime `test_*.gd` 覆盖)
- `python3 scripts/check_v3_runtime_test_coverage.py --report-md docs/plan/v3-runtime-coverage-matrix.md --report-json docs/plan/v3-runtime-coverage-matrix.json` (生成方法级 runtime 覆盖矩阵证据)
