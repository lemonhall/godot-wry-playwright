...
func _init() -> void:
	...
	var open_id = session.open(test_url, open_options)
	...
	if not T.require_ok_response(self, eval_resp, "session.eval title"):
		return
	if not T.require_eq(self, T.parse_json_or_null(String(eval_resp.get("result_json", "null"))), "GWry Session Fixture", "3d title mismatch"):
		return
